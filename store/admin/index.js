...
const actions = {
  ...
  getRooms({ commit, state }, payload) {
    return this.app.apolloProvider.defaultClient
      ...
      .then(({ data }) => {
        commit("setRooms", data.fetchRooms.edges);
        return data.fetchRooms.edges;
      })
      .catch(err => {
        return err;
      });
  ...
  getReports({ commit, state }, payload) {
    return this.app.apolloProvider.defaultClient
      .query({
        ...
        variables: { skip: payload.skip, limit: payload.limit },
        context: {
          headers: {
            Authorization: `Bearer ${Cookie.get("EQUI_AUTH_ADMIN")}`
          }
  ...
  //
  deleteReportedUser({ commit, state }, payload) {
    return this.app.apolloProvider.defaultClient
      .mutate({
        mutation: gql.Admin.removeUser,
  ...
  deleteAdmin({ commit, state }, payload) {
    return this.app.apolloProvider.defaultClient
      ...
      .then(({ data }) => {
        commit(
          "setAdmins",
          state.sub.admins.filter(e => e._id !== payload.adminId)
        );
  ...
  deleteTopic({ commit, state }, payload) {
    return this.app.apolloProvider.defaultClient
      ...
      .then(({ data }) => {
        commit(
          "setTopics",
          state.sub.admins.filter(e => e._id !== payload.topicId)
        );
